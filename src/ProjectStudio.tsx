...
export function ProjectStudio({ slug, completed, onComplete }: { slug: string; completed: boolean; onComplete: () => void }) {
  ...
  return <main className="project-studio" aria-label="Capstone implementation studio">
    <div className="studio-intro"><p className="lesson-kicker">From decisions to a working artifact</p><h1>{spec.title}</h1><h2>Train it. Test it. Run it.</h2><p>{spec.description}</p><ol className="build-steps"><li className={artifact ? 'done' : 'current'}><b>01</b> Code & train</li><li className={artifact ? 'done' : ''}><b>02</b> Evaluate holdout</li><li className={verified ? 'done' : artifact ? 'current' : ''}><b>03</b> Static deployment smoke</li></ol></div>
    ...
    <p className="draft-status" role="status">{saved ? artifact ? 'Code and successful evaluation saved on this device.' : 'Code draft saved on this device. No current passing evaluation.' : verified ? 'Device storage is unavailable. Download your kit before leaving.' : 'Device storage is unavailable. Copy your editor code to a local file before leaving.'} {busy ? 'First run downloads Python; code never leaves your browser. Execution is limited to 20 seconds after loading.' : 'Editing code invalidates the current evaluation and preview.'}</p>
    ...
    {artifact && <section className="evaluation-report"><p className="lesson-kicker">Evaluation passed</p><h3>Evidence, not a training score.</h3><div className="studio-metrics">{Object.entries(artifact.metrics).map(([name, metric]) => <div key={name}><span>{metricName(name)}</span><strong>{metric.toFixed(Number.isInteger(metric) ? 0 : 4)}</strong></div>)}</div><ul>{artifact.checks.map(check => <li key={check}>✓ {check}</li>)}</ul><p>{spec.task === 'regression' ? 'MAE and interval radius are in $1,000. Coverage is a fraction, measured on the test set.' : slug === 'catch-fraud' ? 'Precision, recall, and review share are fractions. Log loss is lower-is-better. The threshold is chosen on validation under a review-capacity limit; future review volume may exceed that limit.' : 'Precision, recall, and flagged share are fractions at a fixed 0.5 threshold. Log loss is lower-is-better. Candidate acceptance uses validation log loss; test metrics report the frozen model.'}</p><button className="button primary" onClick={launch}>Launch portable predictor →</button></section>}
